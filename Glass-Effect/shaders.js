...
// Scale "cover" UVs so the image covers the viewport
...
float fractalGlass(float x) {
  float d = 0.0;
  for (int i = -5; i <= 5; i++) {
    d += displacement(x + float(i) * uGlassSmoothness,
                      uStripesFrequency,
                      uGlassStrength);
  }
  d = d / 11.0;
